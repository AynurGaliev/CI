...
if [[ "$TRAVIS" == "true" ]]; then
    ...
    #Copy local certs
    security import Certs/$DISTRIBUTION_CERT_NAME.p12 -k $KEYCHAIN -P "" -T /usr/bin/codesign
    security import Certs/$DEVELOPMENT_CERT_NAME.p12 -k $KEYCHAIN -P "" -T /usr/bin/codesign
